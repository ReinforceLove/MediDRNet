...
def main(args):
    ...
    train_list_path = 'data/train610.list'
    weights_cls = data_info(train_list_path)
    weights_con = np.ones(len(weights_cls))

    sampler_cls = torch.utils.data.sampler.WeightedRandomSampler(weights_cls, 7500, replacement=True)

    dataset_cls = DRD_dataset('data/DRD610/train', train_list_path, 'classification')
    trainloader_cls = torch.utils.data.DataLoader(dataset_cls, sampler=sampler_cls, num_workers=args.workers,
                                                  batch_size=args.batch_size,
                                                  pin_memory=True)

    dataset1 = DRD_dataset('data/DRD610/train', 'data/test610.list', 'test')
    testloader = torch.utils.data.DataLoader(dataset1, num_workers=0, batch_size=16, pin_memory=True)

    model = CLSNet(args.classes, 512, args.contrast_fea_size).to(args.device)
    ...
    for epoch in range(args.epochs):
        ...
        loss, loss1, loss2 = train(model, trainloader_cls, epoch, criterion1, criterion2, scheduler,
                                   optimizer, args)
        print('\n Epoch: {0}\t'
...
def train(model, trainloader_cls, epoch, criterion1, criterion2, scheduler, optimizer, args):
    model.train()
    losses = AverageMeter()
    losses1 = AverageMeter()
    losses2 = AverageMeter()
    a = 1 - (epoch / args.epochs)
    for batch_idx, data in enumerate(trainloader_cls):
        scheduler(optimizer, batch_idx, epoch)

        img2 = data['img'].to(args.device)
        label2 = data['label'].to(args.device)
        soft = model(img2)
        ...
        print(
            f"epoch:{epoch}, batch:{batch_idx}/{len(trainloader_cls)}, lr:{optimizer.param_groups[0]['lr']:.6f}, loss:{losses.avg:.4f}, loss1:{losses1.avg:.4f}, loss2:{losses2.avg:.4f}")
