...
def main(args):
    ...
    weights_cls = data_info(args.train_list_path)
    ...
    dataset_con = DRD_dataset(args.data_path, args.train_list_path, 'contrast')
    trainloader_con = torch.utils.data.DataLoader(dataset_con,shuffle=True,  num_workers=args.workers,
                                                  batch_size=args.batch_size,
                                                  pin_memory=True)
    dataset_cls = DRD_dataset(args.data_path, args.train_list_path, 'classification')
    trainloader_cls = torch.utils.data.DataLoader(dataset_cls, sampler=sampler_cls, num_workers=args.workers,
                                                  batch_size=args.batch_size,
                                                  pin_memory=True)

    dataset1 = DRD_dataset(args.data_path, args.test_list_path, 'test')
    testloader = torch.utils.data.DataLoader(dataset1, num_workers=args.workers, batch_size=args.batch_size, pin_memory=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--data_path", type=str, default='data/DRD610/train')
    parser.add_argument("--train_list_path", type=str, default='data/test610.list')
    parser.add_argument("--test_list_path", type=str, default='data/train610.list')
    ...
    parser.add_argument('--save', metavar='SAVE', default='saves/dwad.pt', help='saved folder')
